...
Audio-space nearest neighbor utilities for the Spectral Neighbor Plus prototype.

This module provides:
- a helper to load a precomputed FAISS index for audio embeddings
- a get_audio_neighbors(...) function that returns top-K audio-similar tracks
"""
# audio_knn.py
...
def load_audio_index(index_path: str) -> faiss.Index:
    """
    Load a precomputed FAISS index for audio embeddings.
    ...
    if not index_path.exists():
        raise FileNotFoundError(f"FAISS audio index not found at: {index_path}")
    return faiss.read_index(str(index_path))


def get_audio_neighbors(
    seed_idx: int,
    ...
    """
    Retrieve top-K audio-space nearest neighbors for a given track.
    ...
    Args:
        ...
        embeddings: 2D array of audio embeddings (N x D), already normalized.
